...
class HighGraphPreprocessing(object):
    ...
    def run(self, num_nodes: int):
        ...
        if self.method is "greedy":
            self.greedy_high_run(num_nodes)
    ...
    def greedy_high_init(self, init_node: Node):
        ...
        self.unchosen = np.ones(self.g.vcount(), dtype=np.bool)
    ...
    def greedy_high(self, num_nodes: int, init_node: Node):
        self.greedy_high_init(init_node)
        self.greedy_high_run(num_nodes - 1)
...
if __name__ == "__main__":
    for dataset in datasets[0:1]:
        ...
        high_graph_loader.set_method("absolute")
